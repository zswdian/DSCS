$(document).ready(function () {
    $.post('/DisplayInterface/Display/StatisticsRegisterPopulation', function (result) {
        ...
        //var wval = [];
        for (var i = 9; i < 19; i++) {
            wdatas.push(result.data[i].name);
            wvalue.push(result.data[i].count);
            //wval.push(result.data[i].count + 10);
        }
        var tenact = document.getElementById('tenact');
        var teChart = echarts.init(tenact);
