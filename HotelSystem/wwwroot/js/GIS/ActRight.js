$(document).ready(function () {
    $.post('/DisplayInterface/Display/StatisticsRegisterPopulation', function (result) {
        console.log(result);
        var tdatas = [];
        var tvalue = [];
        //var tval = [];
        for (var i = 0; i <9; i++) {
            tdatas.push(result.data[i].name);
            tvalue.push(result.data[i].count);
            //tval.push(result.data[i].count + 10);
        }
        var twChart = document.getElementById('twenact');
        var twChart = echarts.init(twChart);
        ...
        setInterval(function () {
            var dynamicData = [random(), random(), random(), random(), random(), random(), random(), random(), random()];
            twChart.setOption({
                series: [{
