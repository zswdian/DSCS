$(document).ready(function () {
    $.post('/DisplayInterface/Display/StatisticsEmphasesTeenagers', function (result) {
        console.log(result);
        var fxdata = [];
        var fyvalue = [];
        for (var i = 0; i < result.data.length; i++) {
            fxdata.push(result.data[i].name);
            fyvalue.push(result.data[i].count);
        }
        console.log(fyvalue);
        var Focus = document.getElementById('FocusYounth');
        var foChart = echarts.init(Focus);
        // 颜色
        var lightBlue = {
            type: 'linear',
            x: 0,
